...
from app.models import (
    ...
    PlanSuscripcionModel, ProductoTiendaModel, ClienteModel, EntrenadorModel,
    DisciplinaModel, MembresiaClienteModel, PagoFacturaModel, SesionProgramadaModel, ReservaInscripcionModel
)
...
async def seed():
    async with AsyncSessionLocal() as db:
        ...
        roles_data = [
            ...
            {"nombre": "Entrenadores", "descripcion": "Gestión de clases y evaluaciones"},
            {"nombre": "Clientes", "descripcion": "Acceso a reservas"}
        ]
        for r in roles_data:
            exists = await db.execute(select(RolModel).where(RolModel.nombre == r["nombre"]))
        ...
        for a in admins_data:
            exists = await db.execute(select(UsuarioModel).where(UsuarioModel.email == a["email"]))
            if not exists.scalar_one_or_none():
                user = UsuarioModel(
                    username=a["username"],
                    email=a["email"],
        ...
        for c in clientes_data:
            exists = await db.execute(select(UsuarioModel).where((UsuarioModel.email == c["email"]) | (UsuarioModel.cedula == c["cedula"]) ))
            if not exists.scalar_one_or_none():
                user = UsuarioModel(
                    username=c["username"],
                    email=c["email"],
        ...
        for e in entrenadores_data:
            exists = await db.execute(select(UsuarioModel).where((UsuarioModel.email == e["email"]) | (UsuarioModel.cedula == e["cedula"])) )
            if not exists.scalar_one_or_none():
                user = UsuarioModel(
                    username=e["username"],
                    email=e["email"],
        ...
        disciplinas_data = [
            {"nombre": "Jumping", "descripcion": "Clase aeróbica de alta instensidad realizada en un mini tramppolín"},
            {"nombre": "Pilates", "descripcion": "Clase de acondicionamiento físico, respiración y flexibilidad"},
            {"nombre": "Crossfit", "descripcion": "Entrenamiento funcional de alta intensidad"}
        ]
        
        for disc in disciplinas_data:
            exists = await db.execute(select(DisciplinaModel).where(DisciplinaModel.nombre == disc["nombre"]))
            if not exists.scalar_one_or_none():
        ...
        categorias_data = [
            {"nombre": "Cardiovascular", "descripcion": "Máquinas de cardio (cintas, elípticas)"},
            {"nombre": "Musculación", "descripcion": "Máquinas de fuerza guiada"},
            {"nombre": "Peso Libre", "descripcion": "Barras, mancuernas, discos"}
        ]
        for cat in categorias_data:
            exists = await db.execute(select(CategoriaMaquinaModel).where(CategoriaMaquinaModel.nombre == cat["nombre"]))
            if not exists.scalar_one_or_none():
        ...
        cat_ids = {}
        for cat in categorias_data:
            result = await db.execute(select(CategoriaMaquinaModel).where(CategoriaMaquinaModel.nombre == cat["nombre"]))
        ...
        for m in maquinas_data:
            exists = await db.execute(select(MaquinaModel).where(MaquinaModel.nombre == m["nombre"]))
            if not exists.scalar_one_or_none():
        ...
        for p in planes_data:
            exists = await db.execute(select(PlanSuscripcionModel).where(PlanSuscripcionModel.nombre == p["nombre"]))
            if not exists.scalar_one_or_none():
        ...
        for plan in planes_data:
            result = await db.execute(select(PlanSuscripcionModel).where(PlanSuscripcionModel.nombre == plan["nombre"]))
            planes[plan["nombre"]] = result.scalar_one().plan_id

        # Obtener clientes
        clientes_db = {}
        for cliente in clientes_data:
            user = await db.execute(select(UsuarioModel).where(UsuarioModel.email == cliente["email"]))
            user_obj = user.scalar_one_or_none()
            if user_obj:
                cliente_model = await db.execute(select(ClienteModel).where(ClienteModel.usuario_id == user_obj.usuario_id))
        ...
        for mem in membresias_data:
            ...
            if cliente and plan_id:
                plan = (await db.execute(select(PlanSuscripcionModel).where(PlanSuscripcionModel.plan_id == plan_id))).scalar_one()
                ...
                exists = await db.execute(select(MembresiaClienteModel).where(
                ...
                if not exists.scalar_one_or_none():
                    ...
                    pago = PagoFacturaModel(
                        ...
                        metodo_pago="Efectivo" if mem["cliente_username"] == "juan" else "Tarjeta",
        ...
        for prod in productos_data:
            exists = await db.execute(select(ProductoTiendaModel).where(ProductoTiendaModel.nombre == prod["nombre"]))
            if not exists.scalar_one_or_none():
        ...
        disciplinas = {}
        for disc in disciplinas_data:
            result = await db.execute(select(DisciplinaModel).where(DisciplinaModel.nombre == disc["nombre"]))
            disciplinas[disc["nombre"]] = result.scalar_one().disciplina_id

        entrenadores_db = {}
        for ent in entrenadores_data:
            user = await db.execute(select(UsuarioModel).where(UsuarioModel.email == ent["email"]))
            user_obj = user.scalar_one_or_none()
            if user_obj:
                ent_model = await db.execute(select(EntrenadorModel).where(EntrenadorModel.usuario_id == user_obj.usuario_id))
        ...
        for ses in sesiones_data:
            exists = await db.execute(select(SesionProgramadaModel).where(
                SesionProgramadaModel.disciplina_id == disciplinas[ses["disciplina"]],
            ...
            if not exists.scalar_one_or_none():
                nueva_sesion = SesionProgramadaModel(
                    ...
                    entrenador_id=entrenadores_db[ses["entrenador_username"]].entrenador_id,
        ...
        for ses in sesiones_list:
            ...
            for cliente_username, cliente_obj in clientes_db.items():
                if cliente_username in ["juan", "maria"]:   # solo dos clientes reservan
                    exists = await db.execute(select(ReservaInscripcionModel).where(
                        ReservaInscripcionModel.cliente_id == cliente_obj.cliente_id,
