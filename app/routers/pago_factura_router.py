...
from app.schemas.pago_factura_schema import pagoFacturaSalida, pagoFacturaEntrada
from app.services.pago_factura_services import PagoFacturaService
from app.services.cliente_service import ClienteService

router = APIRouter(prefix="/api/v1/pagos", tags=["Módulo de Suscripción Activos"])


def get_service(db: AsyncSession = Depends(get_db)) -> PagoFacturaService:
    cliente_service = ClienteService(db)
    return PagoFacturaService(db, cliente_service)
...
async def crear_pago_factura(
    schema: pagoFacturaEntrada, service: PagoFacturaService = Depends(get_service)
):
