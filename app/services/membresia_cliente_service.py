from app.repositories import MembresiaClienteRepository, ClienteRepository, PlanSuscripcionRepository
...
class MembresiaClienteService:
    ...
    async def saber_estado_membresia(self, cliente_id: int) -> dict:
        ...
        cliente_existe = await self.cliente_repo.get_by_id(cliente_id, id_column="cliente_id")
    ...
    async def crear_membresia_cliente(self, schema: membresiaClienteCrear) -> MembresiaClienteModel:
    
        #vemos si el cliente existe
        cliente_existe = await self.cliente_repo.get_by_id(schema.cliente_id, id_column="cliente_id")
        if not cliente_existe:
            raise NotFoundException(detail="Id de Cliente no encontrado", error_code="ID_NOT_FOUND")
        #vemos si el cliente ya tiene una membresia activa
        membresia_activa = await self.membresia_repo.get_membresia_activa(cliente_id=schema.cliente_id)
        if membresia_activa:
            raise AppException(detail="El cliente ya tiene una membresía activa, no se puede crear otra membresía hasta que la actual expire.", error_code="MEMBRESIA_ACTIVA_EXISTENTE", status_code=400)
        #vemos si existe el plan de suscripcion
        plan_existe = await self.plan_repo.get_by_id(schema.plan_id, id_column="plan_id")
        if not plan_existe:
            raise NotFoundException(detail="Id de Plan de Suscripción no encontrado", error_code="PLAN_ID_NOT_FOUND")
        #calculamos la fecha de fin de la membresia
        fecha_inicio = schema.fecha_inicio if schema.fecha_inicio else datetime.now().date()
        ...
        membresia_info["estado"] = "inactiva" # La membresía se crea como inactiva por defecto, se activará al realizar el pago
