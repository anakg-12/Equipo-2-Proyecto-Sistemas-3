...
class VentaTiendaService:
    ...
    async def registrar_venta(self, schema: VentaCompletaEntrada) -> VentaTiendaModel:
        ...
        for item in schema.items:
                    ...
                    if not producto:
                        raise NotFoundException(detail=f"El producto con ID {item.producto_id} no existe", error_code="PRODUCTO_NOT_FOUND")
    ...
    async def listar_ventas(
        self, cliente_id: int = None, fecha_desde=None, fecha_hasta=None, skip: int = 0, limit: int = 20
    ):
        """Lista ventas con filtros opcionales y con paginacion"""
        return await self.venta_repo.get_all_with_filters(cliente_id, fecha_desde, fecha_hasta, skip, limit)
