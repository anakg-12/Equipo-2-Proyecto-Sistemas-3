...
from app.repositories import SesionProgramadaRepository, UsuarioRepository, EntrenadorRepository, ReservaInscripcionRepository
from app.models import SesionProgramadaModel
from app.schemas.sesion_programada_schema import sesionProgramadaEntrada, sesionProgramadaActualizar
...
class SesionProgramadaService:
    ...
    async def list_sesiones(self, estado: Optional[str] = None, fecha: Optional[date] = None, disciplina_id: Optional[int] = None, skip: int = 0, limit: int = 100):
        if disciplina_id is not None:
            ...
            sesiones = await self.sesion_programada.get_all_sesion(skip=skip, limit=limit, **filtros)
    ...
    async def create_sesion(self, schema: sesionProgramadaEntrada) -> SesionProgramadaModel:
        ...
        sesion_info["estado"] = "programada" #al crearla se debe colocar para postgresql que esta programada la sesion
        return await self.sesion_programada.create(**sesion_info)
    
    #funcion para cambiar el estado de una sesion programada
    async def update_sesion_estado(self, sesion_id: int, schema: sesionProgramadaActualizar) -> SesionProgramadaModel:
        sesion_existente = await self.sesion_programada.get_by_id(sesion_id, id_column="sesion_id")
        if not sesion_existente:
            raise NotFoundException(detail="Id de Sesión Programada no encontrado", error_code="ID_NOT_FOUND")
        
        nuevo_estado = schema.estado.value
        actualizar_info = {"estado": nuevo_estado}

        sesion_actualizada = await self.sesion_programada.update(id=sesion_id, data=actualizar_info, id_column="sesion_id")
    ...
    async def listar_sesiones_usuario(self, usuario_id: int, page: int = 1, limit: int = 100):
        entrenador = await self.entrenador_repo.get_by_usuario_id(usuario_id)
        if not entrenador:
            raise NotFoundException(detail="El usuario no es un entrenador", error_code="ENTRENADOR_NOT_FOUND")
        skip = (page - 1) * limit
        total = await self.sesion_programada.count_by_filtros(entrenador_id=entrenador.entrenador_id)
        sesiones = await self.sesion_programada.get_by_filtros(entrenador_id=entrenador.entrenador_id, skip=skip, limit=limit)
        return total, sesiones