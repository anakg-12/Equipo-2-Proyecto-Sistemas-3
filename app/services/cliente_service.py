...
class ClienteService:
    ...
    async def create_cliente(self, schema: clienteInicial) -> ClienteModel:
        ...
        if not usuario.rol or usuario.rol.nombre != RolesEnum.CLIENTES:
            raise AppException(
                detail="El usuario no tiene el rol de cliente",
                error_code="USER_NOT_CLIENT_ROLE",
                status_code=400,
            )
    ...
    async def update_estado_cliente(
        self, cliente_id: int, schema: clienteActualizar
    ) -> ClienteModel:
        cliente = await self.repo.get_by_id_or_fail(
            cliente_id, id_column="cliente_id", entity_name="Cliente"
        )
        nuevo_estado = schema.activo
        cliente_actualizado = await self.repo.update(
            cliente_id, {"activo": nuevo_estado}, id_column="cliente_id"
        )
        usuario_repo = UsuarioRepository(self.repo.db)
        await usuario_repo.update(
            cliente.usuario_id, {"activo": nuevo_estado}, id_column="usuario_id"
        )
