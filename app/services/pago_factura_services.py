from app.repositories import PagoFacturaRepository, MembresiaClienteRepository, PlanSuscripcionRepository
...
class PagoFacturaService:
    ...
    async def create_pago_factura(self, schema: pagoFacturaEntrada) -> PagoFacturaModel:
        ...
        membresia = await self.membresia_repo.get_by_id(schema.membresia_id, id_column="membresia_id")
        if not membresia:
            raise NotFoundException(detail="Id de Membresía no encontrado", error_code="ID_NOT_FOUND")
        ...
        if not plan:
            raise NotFoundException(detail="Id de Plan de Suscripción no encontrado", error_code="ID_NOT_FOUND")
        #Vemos los costos
        costo_plan = plan.costo
        if schema.monto < costo_plan:
            raise BusinessRuleException(detail="El monto debe ser mayor al costo del plan de suscripcion", error_code="MONTO_INVALIDO")
        # vemos la fecha
        hoy = datetime.now().date()
        if membresia.estado == "activa" and membresia.fecha_fin and membresia.fecha_fin > hoy:
        ...
        await self.membresia_repo.update(membresia.membresia_id, data=datos_actualizar, id_column="membresia_id")
        cliente_id = membresia.cliente_id
        cliente_service = ClienteService(self.repo.db)
        schema_activo = clienteActualizar(activo=True)
        await cliente_service.update_estado_cliente(cliente_id, schema_activo)
