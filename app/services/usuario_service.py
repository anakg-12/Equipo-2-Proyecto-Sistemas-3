...
class UsuarioService:
    ...
    async def authenticate_user(self, username: str, password: str):
        ...
        if not user.activo:
            raise AppException(
                detail="Usuario inactivo", error_code="USER_INACTIVE", status_code=403
            )
        return user
    ...
    async def create_usuario(self, schema: usuarioEntrada) -> UsuarioModel:
        ...
        existe_usuario = await self.usuario_repo.get_by_username(
            schema.username, id_column="username"
        )
        if existe_usuario:
            raise AppException(
        ...
        existe_email = await self.usuario_repo.get_by_email(
            schema.email, id_column="email"
        )
        if existe_email:
            raise AppException(
        ...
        existe_cedula = await self.usuario_repo.get_by_cedula(
            schema.cedula, id_column="cedula"
        )
        if existe_cedula:
            raise AppException(
        ...
        rol = await self.rol_repo.get_by_id_or_fail(
            schema.rol_id, id_column="rol_id", entity_name="Rol"
        )
    ...
    async def update_estado_usuario(
        self, id: int, schema: usuarioActualizar
    ) -> UsuarioModel:
        usuario_existente = await self.usuario_repo.get_by_id_or_fail(
            id, id_column="usuario_id", entity_name="Usuario"
        )
        actualizar_info = schema.model_dump(exclude_unset=True)
        return await self.usuario_repo.update(
            id, actualizar_info, id_column="usuario_id"
        )

    # Creamos una funcion para obtener una lista de usuarios (20 usuarios)
    async def list_usuarios(
        self, usuario_id: Optional[int] = None, skip: int = 0, limit: int = 100
    ):
        if usuario_id is not None:
            usuario = await self.usuario_repo.get_by_id_or_fail(
                usuario_id, id_column="usuario_id", entity_name="Usuario"
            )
