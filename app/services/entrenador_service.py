...
class entrenadorService:
    ...
    async def create_entrenador(self, schema: entrenadorInicial) -> EntrenadorModel:
        ...
        usuario = await self.entrenador_repo.get_by_id_or_fail(
            schema.usuario_id, id_column="usuario_id", entity_name="Usuario"
        )
        ...
        if not usuario.rol or usuario.rol.nombre != RolesEnum.ENTRENADORES:
            raise NotFoundException(
                detail="El usuario no tiene el rol de entrenador",
                error_code="USER_NOT_TRAINER_ROLE",
            )
    ...
    async def update_estado_entrenador(
        self, id: int, schema: entrenadorActualizar
    ) -> EntrenadorModel:
        entrenador_existente = await self.entrenador_repo.get_by_id_or_fail(
            id, id_column="entrenador_id", entity_name="Entrenador"
        )

        actualizar_info = schema.model_dump(exclude_unset=True)
        nuevo_estado = actualizar_info.get("activo")

        entrenador_actualizado = await self.entrenador_repo.update(
            entrenador_existente.entrenador_id,
            actualizar_info,
            id_column="entrenador_id",
        )
        if nuevo_estado is not None:
            usuario_repo = UsuarioRepository(self.entrenador_repo.db)
            await usuario_repo.update(
                entrenador_existente.usuario_id,
                {"activo": nuevo_estado},
