...
class maquinaService:
    ...
    async def create_maquina(self, schema: maquinaCrear) -> MaquinaModel:
        ...
        categoria = await self.categoria_repo.get_by_id_or_fail(schema.categoria_id, id_column="categoria_id", entity_name="Categoria")
    ...
    async def get_maquina(self, id: int) -> MaquinaModel:
        maquina_buscada = await self.maquina_repo.get_by_id_or_fail(id, id_column="maquina_id", entity_name="Maquina")
        return maquina_buscada

    # actualizar el estado operativo de la maquina
    async def update_maquina_estado(
        self, id: int, schema: maquinaActualizar
    ) -> MaquinaModel:
        maquina_buscada = await self.maquina_repo.get_by_id_or_fail(id, id_column="maquina_id", entity_name="Maquina")
        actualizar_info = schema.model_dump(exclude_unset=True)
        return await self.maquina_repo.update(
            id, actualizar_info, id_column="maquina_id"
        )
    ...
    async def list_maquinas(
        ...
        if id is not None:
            maquina = await self.maquina_repo.get_by_id_or_fail(id, id_column="maquina_id", entity_name="Maquina")
            if categoria_nombre is not None:
                categoria = await self.categoria_repo.get_by_nombre(
                    categoria_nombre.lower()
                )
                if not categoria:
        ...
        if categoria_nombre is not None:
            ...
            if not categoria:
                raise NotFoundException(
                    detail="Categoría no encontrada", error_code="CATEGORIA_NOT_FOUND"
                )
            total = await self.maquina_repo.count(categoria_id=categoria.categoria_id)
            maquinas = await self.maquina_repo.get_all(
                skip=skip, limit=limit, categoria_id=categoria.categoria_id
            )
