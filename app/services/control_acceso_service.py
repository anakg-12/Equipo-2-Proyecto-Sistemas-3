...
class ControlAccesoService:
    ...
    async def registrar_y_validar_entrada(self, schema: controlAccesoEntrada) -> ControlAccesoModel:
        ...
        for membresia in cliente_existente.membresias:
            if membresia.estado in ["activa", "por vencer"] and membresia.fecha_inicio <= hoy <= membresia.fecha_fin:
                membresia_valida = True
                break
        if not membresia_valida:
            raise AppException(detail="Membresía no válida para acceso", error_code="MEMBRESIA_INVALIDA", status_code=403)
        acceso_info = {
